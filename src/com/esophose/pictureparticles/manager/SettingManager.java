    ...
    public enum PSetting {
        ...
        private Object getValue() {
            if (this.value == null) {
                ...
                case BOOLEAN:
                    this.value = PictureParticles.getPlugin().getConfig().getBoolean(configPath);
                    break;
                case INTEGER:
                    this.value = PictureParticles.getPlugin().getConfig().getInt(configPath);
                    break;
                case LONG:
                    this.value = PictureParticles.getPlugin().getConfig().getLong(configPath);
                    break;
                case DOUBLE:
                    this.value = PictureParticles.getPlugin().getConfig().getDouble(configPath);
                    break;
                case STRING:
                    this.value = PictureParticles.getPlugin().getConfig().getString(configPath);
                    break;
                case STRING_LIST:
                    this.value = PictureParticles.getPlugin().getConfig().getStringList(configPath);
                    break;
                }
            }
            return this.value;
